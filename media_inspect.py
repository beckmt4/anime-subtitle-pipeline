"""Media inspection utilities using ffprobe.

Provides a structured view of audio and subtitle streams so the pipeline
can select tracks and build multi-language processing graphs. This keeps
ffprobe subprocess details isolated from higher-level logic.
"""
...
@dataclass
class StreamBase:
    index: int               # global stream index in the container
    codec: str
    language: Optional[str] = None  # normalized ISO 639-1 when possible
    raw_language: Optional[str] = None  # original tag value
...
class SubtitleStream(StreamBase):
    is_bitmap: bool = False  # e.g. dvd_subtitle / pgssub require OCR
...
def inspect_media(path: str) -> MediaInfo:
    ...
    Uses a single ffprobe invocation requesting all streams + format data.
    """
    p = Path(path).resolve()
    if not p.exists():
...
def choose_audio_track(media: MediaInfo, preferred_languages: Optional[List[str]] = None) -> int:
    ...
    Args:
        ...
        preferred_languages: list of normalized language codes in priority order

    Returns:
        audio-order index suitable for ffmpeg `-map 0:a:INDEX`
    """
    if not media.audio_streams:
        return 0
    preferred_languages = preferred_languages or ["ja", "jpn"]
    # Normalize input list using same mapping logic
    normalized_pref = []
    for lang in preferred_languages:
        n = _norm_lang(lang) or lang
        normalized_pref.append(n)
    ...
    # Fallback to first
    logger.info(
        "No preferred language match (%s); using first audio track.",
        ",".join(normalized_pref),
    )
    return 0
